...
##### <i class="fas fa-comment-dots"></i> Answer: When size is added as attribute, a warning message says: "Using size for a discrete variable is not advised", because it changes the size of the points deppending on the specie. The same for alpha, and the fields are indistinguishable from each other, being not very visual when it comes to representing the data graphically.

```{r answer1.1}
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, colour = Species, shape = Species, size = Species, alpha = Species)) +
  geom_point()
```
:::
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: fill calculates the relative frequency of each "am" type of car and for type of "cyl", stacking the bars and standardising each bar in order to have the same height.
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: The plot is shown in vertical format, instead of in horizontal way.
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: With the horizontal format we can see the different distribution of the values between the diferent qualities of diamond, because the axis are shared for the 5 plots.
:::
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: Adding the "fill" paramether in "geom_boxplot" parenthesis

```{r diamonds-facets answer 4.1}
ggplot(data = diamonds, mapping = aes(y = carat, x = color, colour = color)) +
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: scale_fill_viridis_d()
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: The text for the subtitle for the plot which will be displayed below the title; the text for the caption which will be displayed in the bottom-right of the plot by default and the text for the tag label which will be displayed at the top-left of the plot by default.
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(cut ~ .) +
  scale_fill_viridis_d() +
  labs(title = "Diamond weight variation", x = "Weight of the diamond", fill = "Quality", subtitle = "Different plots for see the diamond wieght variation", caption = "My first ggplot2 caption", tag = "My first ggplot2 tag") +
  theme_bw()
```
:::
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: The theme_test does not display the grid or any other object that does not provide data information.
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer: When you have a plot in a vector format and you zoom in it, you do not lose resolution and you can render it at any size without losing resolution.

```{r answer 6.1}
p <- ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) + geom_point()

ggsave(filename = "plot_png.png", plot = p, width = 6, height = 4)
ggsave(filename = "plot_svg.svg", plot = p, width = 6, height = 4)
```
:::
...
ggsave(filename = "wrap_exercise.png", plot = p, width = 6, height = 4) # In inches by default
```
:::
...
# Checking the class of popdensity and state
class(midwest$popdensity)
class(midwest$state)

# We create new variables in the dataframe, popdensity_f, that is popdensity converted to factor and state_f, that is state as factor
midwest$state_f <- as.factor(midwest$state)

q <- ggplot(data = midwest, mapping = aes(x = area, y = poptotal, size = popdensity, color = state_f)) +
  geom_point() +
  scale_color_viridis_d() +
  labs(title = "Area Vs. Population", x = "Area", y = "Population", size = "Population density", color = "State", caption = "Source: midwest")  +
  theme_light()
q
ggsave(filename = "area_vs_population.png", plot = q, width = 6, height = 4) # In inches by default
```

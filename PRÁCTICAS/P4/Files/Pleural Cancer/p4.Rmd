...
getwd()
load(file = "data/rse_gene_pleura.Rdata")
dim(rse_gene)
```
...
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: For each gene we have its ID, its length (in base pairs) and its symbol.
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: We observe a certain deviation at the y = 0 limit between raw data and the RPKM normalization, but in the case of the TMM method, the deviation is minimal, so this one is the best normalization method.
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: There is a gene underexpressed, 29 overexpressed and 54427 are not differentially expressed.
...
```{r ggplot1}
ggplot(data = resDF, mapping = aes(x = log2FoldChange, y = -log10(padj))) + geom_point()
```
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: In the new dataset there are `r dim(Entrez_BM)[1]` genes, but only 31 have information, because 3 of them have no information for the `entrezgene_id` column.
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: `r length (Entrez_ids)` genes are keeping.
...
```{r data-frame-fig2}
# ggplot2 plot
filter <- GOresult[GOresult$Count >3, ]
filter
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: We can observe that the most represented GO term is the multicellular organismal process one, followed by system process.
